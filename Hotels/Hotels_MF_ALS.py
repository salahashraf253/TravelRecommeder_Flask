...
def recommendations (model,userID,hotels,city):
    ...
    recsforArr=nrecommendations.join(hotels, on='hotelID').select('userID', 'hotelID', 'name', 'ratings', 'Longitude', 'Latitude','City')
    recsforArr= recsforArr.filter(col('userID') == userID).filter(col('City')==city).orderBy(col('ratings').desc())
    recsforArr.show()
    # Convert recommendations to an array
    user_recs_array = recsforArr.collect()
    # Split row object into two arrays
    hotelID_array = [row.hotelID for row in user_recs_array]
    # rating_array = [row.rating for row in user_recs_array]

    # Print arrays
    return hotelID_array
################ TO-DO ##########################
